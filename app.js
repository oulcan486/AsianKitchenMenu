...
let container=this.document.querySelector(".btn-container")
//load item
...
function displayMenuItems(menuItem){
  let displaymenu=menuItem.map(Item=>{
    ...
    <img src=${Item.img} class="photo" alt="menu-items">
    <div class="menu-info">
      <header>
        <h4 >${Item.title}</h4>
...
function MenuItemButtons(){
  ...
  const categoryButtons=categories.map(
   function (category){
      return `<button class="btn-item" type="button" data-id=${category}>${category}</button>`
    }
  ).join("")
